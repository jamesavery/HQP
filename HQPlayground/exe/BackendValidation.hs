...
import Numeric.LinearAlgebra (sumElements, cmap, magnitude, rows, ident, complex)
...
-- | QuickCheck equality predicate with a counterexample carrying the diff magnitude.
(~~) :: CMat -> CMat -> Property
a ~~ b =
  let d = mDiff a b
...
-- | Random rational with small numerator / small denominator, signed.
genRat :: Gen Rational
genRat = do
...
------------------------------------------------------------------------
-- Newtype wrappers so QuickCheck can shrink-show meaningful things
------------------------------------------------------------------------

-- | Random QOp on a bounded number of qubits and depth (n,d both ≤ 4).
...
instance Arbitrary RandomQOp where
  ...
  -- Shrink toward smaller / simpler ops, preserving the qubit count `n`.
...
-- | Random rotation (axis, θ, qubit count) for testing rotation-specific invariants.
data RotGen = RotGen Int QOp Rational
instance Show RotGen where
  show (RotGen n ax θ) = "RotGen{n=" ++ show n ++ ", θ=" ++ show θ ++ "} axis=" ++ showOp ax
