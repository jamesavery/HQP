...
\newtheorem{quickCheck}{Quick Check}[section]
...
\title{Introduction to Quantum Computing\\
       ...
       {\normalsize Lecture 0 notes --- ATPL 2026}}
\author{James Avery (\texttt{avery@di.ku.dk}) \\ with a tiny bit of assistance from Thomas Mørk (\texttt{tm@math.ku.dk})}
\date{\today}
...
A normalised one-qubit state has the form
\[
  \ket\psi = a\ket0+b\ket1,
  \qquad |a|^2+|b|^2=1.
\]
Because a global phase is physically unobservable, states that differ only by
multiplication by $e^{i\delta}$ represent the same physical state.  We may
therefore choose a representative for which the amplitude of $\ket0$ is real
and nonnegative.
...
\[
  a = e^{i\delta}r,
  \qquad
  b = e^{i\delta}se^{i\varphi},
\]
where $r,s\ge 0$ and $r^2+s^2=1$.  Factoring out the global phase gives
\[
  \ket\psi
  \sim
  r\ket0+s e^{i\varphi}\ket1.
\]
Since $r^2+s^2=1$, there exists an angle $\theta\in[0,\pi]$ such that
\[
  r=\cos\frac{\theta}{2},
  \qquad
  s=\sin\frac{\theta}{2}.
\]
Hence every pure one-qubit state can be written as
\[
  \ket\psi=
  \cos\frac{\theta}{2}\,\ket0
  + e^{i\varphi}\sin\frac{\theta}{2}\,\ket1
\]
for suitable angles
\[
  0\le\theta\le\pi,
...
The pair $(\theta,\varphi)$ are spherical coordinates.  They define the
corresponding point
\[
  \mathbf r=
  \begin{pmatrix}
  ...
  \end{pmatrix}
\]
on the unit sphere in $\mathbb R^3$, called the \emph{Bloch sphere}.  


\begin{quickCheck}[Spherical coordinates]
Does the point parametrized by $(\theta,\varphi)$ lie on the 2-sphere? Does the coordinates cover the entire sphere?
\end{quickCheck}
...
\begin{tikzpicture}[scale=2.6,>=Latex]
...
% z-axis
\draw[->] (0,-1.25) -- (0,1.25);
\node[above] at (0,1.25) {$z$};

% y-axis
\draw[->] (-1.25,0) -- (1.25,0);
\node[right] at (1.25,0) {$y$};

% x-axis (positive x toward lower left)
%\draw[dashed] (0.80,0.55) -- (0,0);
\draw[dashed] (0.55,0.38) -- (0,0);

\draw[->] (0,0) -- (-0.80,-0.55);

\node[below left] at (-0.85,-0.60) {$x$};
...
% |0>, |1>
\fill (0,0.9) circle (0.02);
\draw (0,-0.9) circle (0.02);

\node[left] at (0,0.85) {$\ket0$};
\node[right] at (0,-0.85) {$\ket1$};

% |+i>, |-i>
\fill (1,0) circle (0.02);
\fill (-1,0) circle (0.02);

\node[above right] at (1,0)
  {$\ket{+i}$};
...
% |+>, |->
\fill (-0.445,-0.31) circle (0.02);
\draw (0.45,0.31) circle (0.02);

\node[below right] at (-0.47,-0.33)
  {$\ket+$};

\node[above right] at (0.52,0.26)
  {$\ket-$};
...
\coordinate (psi)  at (0.40,0.72);

% projected point in xy-plane
\coordinate (proj) at (0.40,-0.10);

\draw[very thick,blue,->] (0,0) -- (psi);

\fill[blue] (psi) circle (0.025);
%\node[above left=1pt and 1pt] at (psi) {$\ket{\psi}$};
\node at ($(psi)+(-0.09,0.07)$) {$\ket{\psi}$};

% projection onto equatorial plane
\draw[densely dashed,blue] (psi) -- (proj);
\draw[densely dashed,blue] (0,0) -- (proj);

\fill[blue] (proj) circle (0.015);
...
\draw
(0,0.35)
arc[start angle=90,end angle=61,radius=0.35];
...
\coordinate (xdir) at (-0.19,-0.13);

%\draw
%(-0.19,-0.13)
%arc[start angle=-145,end angle=-10,radius=0.80];


\draw
(-0.21,-0.16)
arc[start angle=263,end angle=300,
    x radius=1,y radius=0.35];
...
\caption{
The Bloch sphere representation of a pure qubit state.
The poles are the computational basis states $\ket0$ and $\ket1$.
The states $\ket\pm=(\ket0\pm\ket1)/\sqrt2$ are eigenstates of $X$,
while $\ket{\pm i}=(\ket0\pm i\ket1)/\sqrt2$ are eigenstates of $Y$.
The angles $\theta$ and $\varphi$ determine the state
$\ket{\psi}=
\cos(\theta/2)\ket0+
e^{i\varphi}\sin(\theta/2)\ket1$.
}
\label{fig:blochsphere}

\end{figure}

The north pole $(0,0,1)$ is $\ket0$ and the south pole $(0,0,-1)$ is $\ket1$.
Equatorial states satisfy $\theta=\pi/2$; for example,
\[
  \ket+ =
  ...
  \frac{\ket0-\ket1}{\sqrt2},
\]
corresponding to the points $(1,0,0)$ and $(-1,0,0)$.

\begin{quickCheck}[Basis states on the Bloch sphere]
Use the Bloch decomposition to check these claims.
\end{quickCheck}

The Bloch coordinates can also be extracted directly from the expectation
values {\color{red}Det er ikke gennemgået!} of the Pauli operators
\[
  X=
...
\]
For a pure state $\ket\psi$, define
\[
  r_x=\bra\psi X\ket\psi,
  \qquad
  r_y=\bra\psi Y\ket\psi,
  \qquad
  r_z=\bra\psi Z\ket\psi.
\]
Equivalently,
\[
  \mathbf r
  ...
  \bigr)
\]
or, writing $\boldsymbol{\sigma}=(X,Y,Z)$,
\[
  \mathbf r
  =
...
gives
\[
  r_x=\sin\theta\cos\varphi,
  \qquad
  r_y=\sin\theta\sin\varphi,
  \qquad
  r_z=\cos\theta,
\]
which agrees with the spherical-coordinate description above.  Thus every
pure one-qubit state corresponds to a unique point on the surface of the
Bloch sphere, and vice versa.

\begin{exercise}[Bloch vectors in QPP]
For a one-qubit state $\ket\psi$, the Bloch vector is
\[
  \mathbf r =
  \bigl(
    \bra\psi X\ket\psi,\,
...
\begin{lstlisting}
expect :: QOp -> StateT -> Double
\end{lstlisting}

that computes the expectation value
\[
  \bra\psi A\ket\psi
\]
of a one-qubit observable $A$.  The result should be real for the operators
$X$, $Y$, and $Z$.

Then define

\begin{lstlisting}
bloch :: StateT -> (Double,Double,Double)
\end{lstlisting}

using your function \lstinline|expect|.

Check your implementation on the states

\begin{lstlisting}
ket [0]
ket [1]
apply (evalOp H) (ket [0])
apply (evalOp H) (ket [1])
\end{lstlisting}

and verify that their Bloch vectors are approximately
...
\begin{lstlisting}
rotate :: QOp -> StateT -> (Double,Double,Double)
rotate u ψ = bloch (apply (evalOp u) ψ)
\end{lstlisting}

and use it to investigate the action of the Pauli gates on Bloch vectors.
...
where \lstinline|plus| is the state $\ket+$ from
Section~\ref{sec:states}.

Use the results to determine which axis is fixed by each Pauli gate.
Explain why

\[
  X:(x,y,z)\mapsto(x,-y,-z),
\]

\[
  Y:(x,y,z)\mapsto(-x,y,-z),
\]

\[
  Z:(x,y,z)\mapsto(-x,-y,z),
\]

and conclude that $X$, $Y$, and $Z$ act as $180^\circ$
rotations of the Bloch sphere about the $x$-, $y$-, and $z$-axes.
\end{exercise}
